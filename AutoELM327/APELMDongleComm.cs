...
namespace AutoELM327
{
    ...
    public class APELMDongleComm : IBluetoothHandler, IATCommands
    {
        ...
        // Date          : 20-08-20
        //----------------------------------------------------------------------------
        public async Task<object> Dongle_SetProtocol(ProtocolEnum protocolEnum)
        {
            Debug.WriteLine("------Dongle_SetProtocol------", DebugTag);
            //ATSPx
            var command = "ATSP" + ((int)protocolEnum).ToString();
        ...
        public async Task<ResponseArrayStatus> CAN_TxRx(int frameLength, string txrx)
        {
            try
            {
                ...
                if (actualResponse.Contains("BUSINIT:ERROR"))
                {
                    responseStructure = new ResponseArrayStatus
                    {
                        ECUResponse = null,
                        ECUResponseStatus = "ERROR : BUS INIT",
                        ActualDataBytes = null
                    };
                }
                else if (actualResponse.Contains(" ") || (actualResponse == "NODATA") || (actualResponse == "CANERROR"))
                {
                    responseStructure = new ResponseArrayStatus
                    {
                        ECUResponse = null,
        ...
        private string ExtractResponse(object CAN_TxRxData)
        {
            ...
            foreach (string item in responseArray)
            {
                ...
                else if(item.Contains("BUS INIT: ERROR"))
                {
                    output = item;
                }
