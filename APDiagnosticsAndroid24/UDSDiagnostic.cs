...
//using APDiagnosticsAndroid24.Enums;
using APDiagnosticsAndroid24.Helper;
//using APDiagnosticsAndroid24.Models;
using APDiagnosticsAndroid24.Structures;
...
namespace APDiagnosticsAndroid24
{
    public class UDSDiagnostic
    {
        #region Properties
        ReadDTCIndex DTCIndex = 0;
        private DongleComm dongleCommWin;
        private ECUCalculateSeedkey calculateSeedkey;
        #endregion

        #region CTOR
        public UDSDiagnostic(DongleComm dongleCommWin)
        {
        ...
        public async Task<string[]> genericOBDSupportedPidList()
        {
            try
            {
                ...
                {
                    var frameLength = 2;
                    var response = await this.dongleCommWin.CAN_TxRx(frameLength, "01" + (i).ToString("X2"));
        ...
        public async Task<ObservableCollection<WriteParameterResponse>> WriteParameters(int noOFParameters, WriteParameterIndex writeParameterIndex, ObservableCollection<WriteParameterPID> writeParameterCollection)
        {
            ...
            foreach (var pidItem in writeParameterCollection)
            {
                ...
                if (pidResponse.ECUResponseStatus == "NOERROR")
                {
                    ...
                    if (status == "NOERROR")
                    {
                        ...
                        Byte seedkeylen = 0;
                        if ((pidItem.seedkeyindex == APDiagnosticAndroid.Enums.SEEDKEYINDEXTYPE.GREAVES_BOSCH_BS6_PROD) ||
                            (pidItem.seedkeyindex == APDiagnosticAndroid.Enums.SEEDKEYINDEXTYPE.GREAVES_BOSCH_BS6_DEV))
                        {
                            seedkeylen = 4;
                        }
                        else
                        ...
                        if (status == "NOERROR")
                        {
                            ...
                            if (pidResponse3.ECUResponseStatus == "NOERROR")
                            {
                                ...
                                if (pidItem.ReadParameterPID_DataType == "IQA")
                                {
                                    ...
                                    Array.Copy(iqa_x, 0, writeFrame, writeParaPID.Length + 1, 8);
                                }
                                else
                                {
                                    tx_Frame_len = 1 + writeParaPID.Length + pidItem.writeparadata.Length;
            ...
            foreach (var item in WriteParameterCollection)
            {
            ...
            for (int i = 0; i < noofsectors; i++)
            {
                ...
                for (int j = 0; j < sector_num_bytes;)
                {
                    try
                    {
                        ...
                        //await GetRuntimeFlashPercent();

                        //runtimeflashpercent = (float)realtimebytesflashed / (float)totalbytestobeflashed;

                        //MessagingCenter.Send<UDSDiagnostic, float>(this, "RealTimeBytesFlashed", runtimeflashpercent);
